...
app.get("/hotels/:location/:filter", (req, res) => {
  res.end(
    `It will list all hotels in ${req.params.location} that is ${req.params.filter}`
  );
});
...
app.get("/continent", (req, res) => {
  res.render("continent/", { arrayOfData });
});
app.get("/continent/:regionName", (req, res) => {
  const regionId = region(req.params.regionName.toLowerCase());
  res.render("continent/showRegion", { regionId });
});
app.get("/continent/:regionName/countries", (req, res) => {
  const c = countries(req.params.regionName.split("%20").join(" "));
  res.send(c);
});

app.get("/continent/:regionName/country/:countryName", (req, res) => {
  const country = countryByName(req.params.regionName, req.params.countryName);
